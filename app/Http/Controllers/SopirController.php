...
class SopirController extends Controller
{
    ...
    // Complete order (jika status sudah dikonfirmasi oleh penumpang)
    public function completeOrder(Request $request, $id)
    {
        ...
        try {
            ...
            if (!$assignment) {
                throw new Exception('Assignment tidak ditemukan untuk order ini');
            }

            // Hanya bisa complete order assignment sendiri
            if ($assignment->sopir_id !== $user->pengguna_id) {
                throw new Exception('Anda tidak memiliki akses ke order ini');
            }

            // Hanya bisa complete jika status CONFIRMED
            if ($order->status !== Order::STATUS_CONFIRMED) {
                throw new Exception(
                    'Pesanan belum dikonfirmasi penumpang'
                );
            }
            $sopir = Sopir::lockForUpdate()->findOrFail($assignment->sopir_id);
            $mobil = Mobil::lockForUpdate()->findOrFail($assignment->mobil_id);
