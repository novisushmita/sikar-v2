...
const API_URL = "{{ url('/api/penumpang/orders') }}";
...
// Backend: order.review adalah array dari ReviewSopir (hasMany via Order->review)
// Kita ambil review pertama yang ada (setiap order hanya 1 review dari penumpang ini)
function renderReviewBadge(order) {
    // review diload sebagai relasi di Order model (with(['review']))
    // Bisa berupa array atau object tergantung relasi
    ...
    // Hanya tampilkan review untuk order yang sudah selesai/confirmed
    const showReviewStatuses = ['completed', 'confirmed'];
    if (!showReviewStatuses.includes(order.status)) {
        return '';
    }
...
async function fetchOrders() {
    ...
    try {
        const response = await fetch(`${API_URL}?status=all&token=${authToken}`, {
            method: 'GET',
            headers: {
        ...
        if (result.status) {
            ...
            nomorKepalaSopir = result.data?.[0]?.nomor_kepala_sopir || null;

            // ✅ Urutkan dari pemesanan terbaru
...
function applyFilters() {
    ...
    filteredOrders = allOrders.filter(order => {
        const validStatuses = ['assigned', 'confirmed', 'completed', 'canceled', 'rejected'];
...
function createOrderCard(order) {
    ...
    const waktuDiperbarui  = formatDateTime(order.created_at);
    const waktuPembuatan   = formatDateTime(order.updated_at);
    ...
    // Tombol WA
    const showWa = ['assigned', 'on-process', 'confirmed', 'completed'].includes(order.status) && order.assignment;
    const waButtons = showWa ? `
        <div class="flex gap-2 mt-3">
            ${nomorKepalaSopir ? `
            <a href="https://wa.me/${cleanPhone(nomorKepalaSopir)}" target="_blank"
...
document.getElementById('resetFilter').onclick = () => { document.getElementById('dateFilter').value = getTodayDate(); applyFilters(); };
