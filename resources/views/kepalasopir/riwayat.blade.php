...
function getStatusConfig(status) {
    const configs = {
        ...
        'on-process': { label: 'Diperjalanan',    bgColor: 'bg-yellow-50', textColor: 'text-yellow-700', borderColor: 'border-yellow-200' },
        ...
        'rejected':   { label: 'Ditolak',         bgColor: 'bg-orange-50', textColor: 'text-orange-700', borderColor: 'border-orange-200' },
...
function applyFilters() {
    ...
    filteredOrders = allOrders.filter(order => {
        ...
        if (dateFrom && dateTo) {
            ...
            return order.waktu_penjemputan.substring(0, 10) >= dateFrom;
        } else if (dateTo) {
            return order.waktu_penjemputan.substring(0, 10) <= dateTo;
        }
        return true;
    });

    if (dateFrom && dateTo) {
        document.getElementById('dateRangeInfo').textContent = `${formatDate(dateFrom)} - ${formatDate(dateTo)}`;
    }
...
function createOrderCard(order) {
    ...
    const waButtons = (sopirNomor || penumpangNomor) ? `
        <div class="flex gap-2 mt-3">
            ${sopirNomor ? `
            <a href="https://wa.me/${sopirNomor}" target="_blank"
               class="flex-1 flex items-center justify-center gap-1.5 bg-green-500 hover:bg-green-600
                      text-white font-semibold text-xs py-2.5 rounded-lg transition-all shadow-sm hover:shadow-md">
                ${waSvg} Sopir
            </a>` : ''}
            ${penumpangNomor ? `
            <a href="https://wa.me/${penumpangNomor}" target="_blank"
               class="flex-1 flex items-center justify-center gap-1.5 bg-green-500 hover:bg-green-600
                      text-white font-semibold text-xs py-2.5 rounded-lg transition-all shadow-sm hover:shadow-md">
                ${waSvg} Penumpang
            </a>` : ''}
    ...
    return `
        <div class="border border-gray-200 rounded-lg md:rounded-xl p-4 md:p-5 transition-all">
            ...
            <div class="flex justify-between items-start mb-4">
                <div>
                    <p class="text-xs text-gray-500">ID Pemesanan <span class="font-semibold text-gray-700">#${order.order_id}</span></p>
...
function showNotification(message, type = 'info') {
    ...
    notification.className = `fixed top-4 right-4 z-50 px-5 py-3 rounded-lg shadow-lg transform transition-all duration-300 translate-x-full ${bgColor}`;
...
function showExportModal(dateFrom, dateTo) {
    document.getElementById('exportModalMessage').textContent = `Ekspor data dari ${formatDate(dateFrom)} sampai ${formatDate(dateTo)}?`;
    document.getElementById('confirmExportModal').classList.remove('hidden');
}

function hideExportModal() {
    document.getElementById('confirmExportModal').classList.add('hidden');
}
...
document.getElementById('cancelExportBtn').onclick = () => hideExportModal();
...
document.getElementById('confirmExportBtn').onclick = async () => {
    ...
    exportBtn.innerHTML = `<svg class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Mengexport...`;
    ...
    try {
        ...
        const response = await fetch("{{ url('/api/kepalasopir/export') }}", {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${authToken}`, 'Accept': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' },
            body: formData
        });
...
document.getElementById('confirmExportModal').onclick = (e) => {
    if (e.target.id === 'confirmExportModal') hideExportModal();
};
